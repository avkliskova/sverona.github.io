...
A full exposition of the theory of Lie algebras and their representations is well beyond the scope of this paper. We provide here only the necessary definitions and theorems to read §3.^[Sidenote to debug] 
 
We loosely follow [@humphreys]. An introduction to the subject more suitable for advanced undergraduates is [@erdmann-wildon].
...
::: proof
 Let \\(M, M'\\)
be \\(n \times n\\) matrices.^[It suffices to show that if two matrices each have zero trace, so does their commutator. In fact the commutator of any two matrices has zero trace, which is what we actually show.]
Then \\[\begin{aligned}
...
\\[ SM = \begin{bmatrix}1 & 0 & 0 \\\\ 0  & 0 & I\_k \\\\ 0 & I\_k & 0\end{bmatrix}M \\]
...
\\[SM = \begin{bmatrix}0 & I\_k \\\\ I\_k & 0\end{bmatrix}M\\] is skew-symmetric forms the *even
...
the matrix \\(
SM = \begin{bmatrix} \mathbf 0\_n & \mathbf I\_n \\\\ -\mathbf I\_n & \mathbf 0\_n \end{bmatrix}M
\\) is symmetric. 
:::
...
[H]
\centering
\subfloat[\(A\_1,\) rank 1]{%
\includegraphics{diagrams/a1.pdf}
}\hspace{0.1\textwidth}
\subfloat[\(A_2,\) rank 2]{%
\includegraphics{diagrams/a2.pdf}
}\\
\subfloat[\(B_2,\) rank 2]{%
\includegraphics{diagrams/b2.pdf}
}\hspace{0.1\textwidth}
\subfloat[\(G_2,\) rank 2]{%
\includegraphics{diagrams/g2.pdf}
}
\caption{Some root systems, their ranks, and their bases.}
...
::: proof
 Let
\\(z \in Z(\mathfrak g).\\) \\(\operatorname{div} z,\\) the one-dimensional
...
simple, this implies \\(\operatorname{div} z = \{0\}.\\) Thus \\(z = 0.\\)
...
\\(\mathfrak h = \operatorname{diag} A\_1 = \operatorname{div} (1, -1)\\) is
...
3.  The dual space \\(\mathfrak h^\*\\) is (\\(\mathbb F\\)-)divned by \\(\Phi.\\)
...
Similarly, \\([hy] = -2y.\\) Therefore \\(\operatorname{div}\_{\mathbb{F}} \{x, y, h\} \approx A\_1.\\)
...
\\(\operatorname{div}\_{\mathbb F} x = \mathfrak g\_\alpha.\\) Similarly, we
have \\(\operatorname{div}\_{\mathbb F} y = \mathfrak g\_{-\alpha}.\\)
Finally, by consequence 5 of Theorem 2.5,
\\(\operatorname{div}\_{\mathbb F} h = [\mathfrak g\_{\alpha}, \mathfrak g\_{-\alpha}].\\)
...
::: proof
 Let
\\(E\_{\mathbb Q} \coloneqq \operatorname{div}\_{\mathbb Q}\Phi.\\) By
...
Furthermore, we have \\(E = \operatorname{div}\_{\mathbb R} \Phi.\\) Also:
...
obtained depends only on the choice of Lie algebra. See [@humphreys].
...
\beta = \begin{bmatrix} -1/2 \\\\ \sqrt3/2\end{bmatrix}.\\] We compute
...
Representation theory gives us another way to think of, and a much simpler way to compute, the roots of a semisimple Lie algebra. For the remainder of this section, assume \\(\mathfrak g\\) is semisimple over an algebraically closed field \\(F\\) of characteristic zero.^[Taking \\(F = \mathbb R\\) will suffice.] 
...
spaces are the \`\`weight spaces.\'\'^[Sidenote at the end of a para but outside a div with no later element or something, I'm really confused]
...
\\(A\_2\\) with respect to its diagonal subalgebra.^[Here's a sidenote at the end of a para so I can test.]
...
x = \begin{bmatrix} a & b & c \\\\ d & e & f \\\\ g & i & -(a + e)\end{bmatrix} \mapsto [hx] = \begin{bmatrix}0 & (h\_1 - h\_2)b & (2h\_1 + h\_2)c \\\\ (h\_2 - h\_1)d & 0 & (h\_1 + 2h\_2)f \\\\ (-2h\_1 - h\_2)g & (-h\_1-2h\_2)i & 0\end{bmatrix}.
\end{aligned}$$

We can put this in matrix form:
\\[\operatorname{ad} h = \operatorname{diag} \begin{bmatrix} 0\\\\ h\_1 - h\_2\\\\ 2h\_1 + h\_2\\\\ h\_2 - h\_1\\\\ 0\\\\ h\_1 + 2h\_2\\\\ -2h\_1 - h\_2\\\\ -h\_1 - 2h\_2\end{bmatrix}.\\]
...
\centering
\includegraphics{diagrams/fake-a2.pdf}
\caption{The not-quite-a-root-system given by the roots of \(A\_2\) with respect to \(\operatorname{diag} A_2.\)}
...
::: definition
 Let \\(\Lambda\\) be the \\(\mathbb Z\\)-div of
...
1.  If \\(r \overset{i}\to s\\) in \\(R,\\) then
    \\(\omega(s) - \omega(r) = \alpha_i.\\)
2.  \\(\langle \omega(r), \alpha_i^\vee \rangle = 2\rho_i(r) - l_i(r),\\)
    where \\(\rho_i(r),\\) \\(l_i(r)\\) are respectively the rank of \\(r\\) within
    its \\(i\\)-component and the length of that \\(i\\)-component.
3.  Define the weight-generating function of \\(R\\) as

$$
WGF(R; \mathbf z) \coloneqq \sum_{r \in R} \mathbf z^{\omega(r)} = \sum_{r \in R} \prod_{i=1}^n z_i^{\langle \omega(r), \alpha_i^\vee \rangle}.$$
...
::: proof
 See [@donnelly-extremal] and
[@donnelly-poset-models]. 
:::
